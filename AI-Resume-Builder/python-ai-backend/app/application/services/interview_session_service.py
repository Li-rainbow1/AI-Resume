...
def persist_turn_result(
    ...
    memory_summary = str(graph_output.get("memorySummary") or "").strip()
    ...
    cached_request_id = str(session.get("lastRequestId") or "").strip()
    cached_response = session.get("lastResponse")
    if request_id and request_id == cached_request_id and isinstance(cached_response, dict):
        return cached_response
    completed_ids = list(session.get('completedRequestIds') or [])
    if cached_request_id and cached_request_id not in completed_ids:
    ...
    if int(session.get('contextVersion') or 0) != expected_version:
        raise ValueError('会话已更新，请重新加载后重试')
    session['contextVersion'] = expected_version
    session['summaryThroughSeq'] = int(graph_output.get('summaryThroughSeq') or 0)
    ...
    session["lastRequestId"] = request_id or None
    session["lastResponse"] = done_payload
    # 已完成标识随会话与消息一起提交；摘要保存不会清空此列表。
    if request_id:
        completed_ids.append(request_id)
    session['completedRequestIds'] = completed_ids
...
def _session_to_detail(session: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        # 旧响应字段保留为空，避免历史客户端因字段缺失报错；不再读取数据库中的手动范围。
        "projectIds": [],
        "messages": session.get("messages", []),
        "createdAt": session.get("createdAt", ""),
        "updatedAt": session.get("updatedAt", ""),
