...
const pendingExternalValue = ref<string | null>(null)
...
// 编辑器自身触发的 v-model 回显不再写回 DOM，避免破坏浏览器原生撤回历史。
watch(() => props.modelValue, (value) => {
  const nextValue = value || ''
  if (nextValue === lastEmittedValue.value) {
    showPlaceholder.value = !nextValue
    return
  }
  if (isFocused.value) {
...
function onBlur() {
  ...
  const nextValue = pendingExternalValue.value
  pendingExternalValue.value = null
  syncEditorValue(nextValue)
  lastEmittedValue.value = nextValue
}
