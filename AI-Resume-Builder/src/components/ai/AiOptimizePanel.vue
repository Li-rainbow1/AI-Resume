...
type ApplyUndoSnapshot = {
  ...
  previousProjectDescriptions?: string[]
...
// 解析项目经历优化结果：按项目分段，每段收敛为单一工作描述内容
function parseProjectOptimizedSections(markdownText: string, projectList: ProjectEntry[]): string[] {
...
function isWorkMetadataLine(line: string, workList: WorkEntry[]): boolean {
  ...
  if (/^实习经历\s*\d+$/i.test(plain)) return true
  if (/^(公司|单位|项目名称|部门|职位|岗位|任职时间|时间)\s*[:：]/.test(plain)) return true
...
function parseWorkOptimizedContent(markdownText: string, workList: WorkEntry[]): string[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const hasMetaSeparator = /[|｜•·]/.test(plain)
    const headingCandidate = normalizeHeadingLineCandidate(raw)
    const isSectionHeading = /^#{1,6}\s*实习经历\s*\d+/i.test(headingCandidate) || /^实习经历\s*\d+[：:]?$/i.test(plain)
    ...
    case 'projectExperience':
      if (resumeStore.projectList.length > 0 && resumeStore.projectList[0]) {
        ...
        undoSnapshot = {
          ...
          previousProjectDescriptions: resumeStore.projectList.map((p) => p.description),
        }
        ...
        for (let i = 0; i < maxApplyCount; i++) {
          ...
          const enhancedDescription = enhanceProjectMainWorkBold(section)
          project.description = renderProjectMainWorkWithOriginalStyle(enhancedDescription, project.description)
          applied = true
        }

        if (!applied && content.trim()) {
          const enhancedDescription = enhanceProjectMainWorkBold(optimizedContent)
          target.description = renderProjectMainWorkWithOriginalStyle(enhancedDescription, target.description)
          applied = true
        }
      }
      break
...
function handleUndoApply() {
  ...
  switch (key) {
    ...
    case 'workExperience':
      ...
      break
    case 'projectExperience':
      if (snapshot.previousProjectDescriptions?.length) {
        const restoreCount = Math.min(snapshot.previousProjectDescriptions.length, resumeStore.projectList.length)
        for (let i = 0; i < restoreCount; i++) {
          const target = resumeStore.projectList[i]
          if (!target) continue
          target.description = snapshot.previousProjectDescriptions[i] ?? ''
        }
      } else if (resumeStore.projectList.length > 0 && resumeStore.projectList[0]) {
        resumeStore.projectList[0].description = snapshot.previousContent
      }
      break
    case 'awards':
