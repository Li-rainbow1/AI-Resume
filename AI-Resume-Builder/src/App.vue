...
import SystemServiceConfigPanel from '@/components/settings/SystemServiceConfigPanel.vue'
import {
...
function resolveAllowedMenuKey(key: PrimaryMenuKey): PrimaryMenuKey {
  ...
  if (key === 'system-services' && !authStore.isAdmin) {
    return DEFAULT_PRIMARY_MENU_KEY
  }
  return key
}
...
function handleSelectMenu(key: PrimaryMenuKey) {
  ...
  const allowedKey = resolveAllowedMenuKey(key)
  setActiveMenu(allowedKey)

  if (typeof window === 'undefined') return

  const targetPath = resolvePrimaryMenuPath(allowedKey)
  if (normalizePrimaryRoutePath(window.location.pathname) !== targetPath) {
    window.history.pushState({ primaryMenu: allowedKey }, '', targetPath)
  }
}
  ...
  <div v-else class="app-layout">
    ...
    <div class="main-content">
      ...
      <AccountSettingsPanel
        ...
        @logout="handleLogout"
      />
      <SystemServiceConfigPanel
        v-if="activeMenu === 'system-services' && authStore.isAdmin"
      />
    </div>
  </div>
</template>
