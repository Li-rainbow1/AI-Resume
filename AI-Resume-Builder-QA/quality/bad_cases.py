...
def classify_bad_case(
    ...
    if case.question_type in {"image_ocr", "table_or_flow", "mixed"} and metrics["image_knowledge_hit_rate"] < 1:
        categories.append("图片Chunk未命中")
        reasons.append("sources 未命中预期 image_vision Chunk")
    if document_hit and metrics["recall_at_k"] < 1:
    ...
    if expected_in_source and metrics["fact_coverage_rate"] < 1:
    ...
    if case.question_type == "no_answer" and metrics["no_answer_rejection_rate"] < 1:
