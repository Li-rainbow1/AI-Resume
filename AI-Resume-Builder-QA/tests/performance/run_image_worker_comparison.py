...
# 性能编排代码已按测试类型收敛到 tests/ 下：直接执行、被 runpy 拉起或被 subprocess 调起时，
# 都必须先把仓库根放入搜索路径，才能以命名空间包 tests.* 导入。
if str(ROOT) not in sys.path:
    sys.path.insert(0, str(ROOT))

from tests.performance.model_config import build_model_environment, model_summary, selected_mode  # noqa: E402
from tests.performance.locustfiles.common.data_factory import IMAGE_COUNT, PerformanceDataFactory  # noqa: E402


BUSINESS_ROOT = ROOT.parent / "AI-Resume-Builder"
COMPOSE_FILE = ROOT / "tests" / "performance" / "image_parser_stack.compose.yml"
# legacy 基线 tree：旧版快照 5901d9ac 补入视觉解析重试修复后的快照，取舍原因见 tests/performance/README.md。
    ...
    try:
        ...
        locust_result = _run(
            [
                ...
                "locust",
                "-f",
                "tests/performance/locustfiles/image_worker_comparison.py",
                "--headless",
                "--host",
