...
pytest + httpx、本地隔离 QA 环境、单条 Playwright UI 主链、两项收敛后的性能编排及第一版 AI/RAG 质量评测脚本已建立。历史五项 Locust 短时冒烟记录仍保留；真实模型质量基线尚未执行，正式性能基线已于 2026-09-12 执行（图片解析同步串行 vs 异步并发 3）。隔离环境使用独立容器、网络、端口和数据卷，不连接现有业务数据库。
